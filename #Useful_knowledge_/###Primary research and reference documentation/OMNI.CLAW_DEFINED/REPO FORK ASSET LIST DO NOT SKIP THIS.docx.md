Important assets;

llama.cpp-npu, EdgeAIApp-ExecuTorch, c10vis-poem/mlc-llm,
c10vis-poem/genymotion-device-web-player

c10vis-poem/off-grid-ai-mobile

c10vis-poem/qcom-build-utils

c10vis-poem/snapdragon-npu-llm

c10vis-poem/docker-pkg-build

Important on device layers that I will be running;

c10vis-poem/tmux , c10vis-poem/claude-code-android

c10vis-poem/termux-packages

c10vis-poem/OmniRoute

c10vis-poem/tmux-assistant-resurrect

Very important for proof of concept, reverse engineering, and
troubleshoot fall back also research guide:

c10vis-poem/off-grid-ai-mobile

c10vis-poem/VarientDemoApplication

Magisk

{This is Genymobile, her entire repo and website deserve scraping This
will not be the last time you hear me mention her or see a resource of
hers posted if I were you I would task agents of scrubbing her entire
online and GitHub libraries\]

edge-overlay-ai

nexa-sdk

I really don't think this works anymore but it could be worth looking at
for reverse engineering and maybe it does support some active API key
but I dk

c10vis-poem/scrcpy

Resources ;

qcom-build-utils adk-docs c10vis-poem/AI-Agents-Projects-Tutorials

trusted-firmware-a **ai-hub-apps**

Skills ; Don't sleep on these!

SuperClaude Framework // Claude-skills //

leaked-system-prompts //

Prompt-eng-interactive-tutorial //

c10vis-poem/claude-code-best-practice

Probably the most important skill this right here teaches Claude how to
reverse engineer Android apps;

c10vis-poem/claude-android-skill

c10vis-poem/android-reverse-engineering-skill

Voice and memory layers

\[ most likely get the version best suited to match not sure of its
tflite\]

moonshine-tflite c10vis-poem/silero-vad

Kokoro

OB1 reasoning-bank

Honorable mention;

c10vis-poem/kernel-topics

c10vis-poem/snagboot
