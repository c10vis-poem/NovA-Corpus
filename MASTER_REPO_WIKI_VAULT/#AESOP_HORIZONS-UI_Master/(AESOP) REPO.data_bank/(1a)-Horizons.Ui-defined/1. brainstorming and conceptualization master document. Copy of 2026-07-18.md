...
What is the core engine initiating the run? This parameter defines the executable binary (such as \`ort\_engine\`, \`geniex\`, or a custom script). The system simply verifies that this target exists in your app directories or downloads, and checks if it possesses the necessary executable permissions.  
  
\*\*2. Required Assets (The Dependencies)\*\*  
What additional pieces does the binary need to operate? This defines a list of mandatory companion files, such as the selected model file itself, or specific SDK libraries (like the four QNN libs needed to run \`ort\_engine\`).   
...
.  
To cover the broadest horizons—while specifically addressing your goals of avoiding low-memory crashes and incompatible hardware—here is the universal checklist that should constitute a runtime's info packet:  
1\. The Execution Target (Binary) What is the core engine initiating the run? This parameter defines the executable binary (such as ort\_engine, geniex, or a custom script)  
...
.  
2\. Required Assets (The Dependencies) What additional pieces does the binary need to operate? This defines a list of mandatory companion files, such as the selected model file itself, or specific SDK libraries (like the four QNN libs needed to run ort\_engine)  
.  
...
This is the core motor. We just need to know what vehicle we are trying to start, whether that is \`ort\_engine\`, \`geniex\`, or a custom script.   
...
1\. \*\*Execution Target (\`binaryName\`):\*\* The core engine (e.g., \`ort\_engine\` or \`geniex\`).   
...
Agents building new UI handlers or integrations must adhere to the fluid slot architecture:  
\* \*\*Slot counts are not fixed at 3:\*\* A cloud API config requires 3 slots (endpoint, key, model), but a pure terminal script requires only 1, and an on-device CLI requires 0.   
\* \*\*Always provide an escape hatch:\*\* Every slot in every room must allow the user to manually "add / upload / plug in / write a script". The app must accommodate custom inputs graceful
