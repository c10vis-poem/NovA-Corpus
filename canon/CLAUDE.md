...
### WHO
The operator is the only source of canon. Agents transcribe; they do not author it.

### WHAT
The blueprint, the three LOCKED GUI specs (Router stereo stack, Monitor arcade cabinet, Terminal matrix cascade), HOME-REDESIGN-SPEC and its 44 reference images, the parameter packet, the state-of-existence ledger.

### WHERE
Top of the corpus. Everything else trickles down from here.

### WHEN
Read before anything else, every session.

### WHY
Because every other document described the *designed* system as though it were *built*. Canon is the fixed point that makes the rest checkable.

### HOW
Operator's verbatim word, anchored to a concrete artifact, derived content visibly separated, build state declared. See `TEMPLATE.md`.

---

## Delta

**The blueprint is still DRAFT** pending the joint canon pass. §17 grades every source it was assembled from.
